...
#%% EXP Extract box coordinates to display percentiles in scatter plot
stats = []
...
stats.append([x_min, x_max, x_iqr, y_min, y_max, y_iqr]) 
...
ax = fig.add_subplot(111)
title = 'Scatter ALL timesteps with fliers with labeling in years zoomed min max' #  zoomed min max
...
for j in range(0,3):
    ...
    ax.scatter(x_val, y_val, marker='x', color = color, label=label, alpha = 0.5) # , alpha = 0.5
    for i,type in enumerate(types):                         # uncomment these lines to turn off labeling
        x = x_val[i]                                        #
        y = y_val[i]                                        #
        plt.text(x+0.02, y+0.02, type, fontsize=7)            #
plt.title(title)
